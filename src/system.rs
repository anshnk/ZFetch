...
pub async fn get_system_info(config: &Config) -> SystemInfo {
    ...
    let sys_task = tokio::task::spawn_blocking(|| {
        // Only refresh the data we actually need, not everything including processes
...
async fn get_storage_info() -> Vec<StorageInfo> {
    ...
    {
        use tokio::process::Command;
        if let Ok(output) = Command::new("df").arg("-k").arg("/").output().await {
            let stdout = String::from_utf8_lossy(&output.stdout);
        ...
        loop {
            ...
            .is_ok()
            {
                let name = String::from_utf16_lossy(
                    &buffer[..buffer.iter().position(|&c| c == 0).unwrap_or(0)],
                );
